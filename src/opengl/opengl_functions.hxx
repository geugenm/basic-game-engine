...
SDL_Window *get_new_sdl_window(const char *window_title, const int &height, const int &width);
...
bool file_has_changed(const std::string &file_path, std::time_t &last_modified_time);

std::vector<GLfloat> get_vertices_from_glsl_file(const std::string &shader_path);

GLuint get_compiled_shader(GLenum shader_type, const GLchar *shader_content);
...
void generate_vertex_array(GLsizei generated_names_amount, GLuint *buffer_array);

void generate_buffer_object_name(GLsizei generated_names_amount, GLuint *buffer_array);
