...
//  2022-03-22: Inputs: Fix mouse position issues when dragging outside of boundaries.
//  SDL_CaptureMouse() erroneously still gives out LEAVE events when hovering OS decorations.
//  2022-03-22: Inputs: Added support for extra mouse buttons (SDL_BUTTON_X1/SDL_BUTTON_X2).
//  2022-02-04: Added SDL_Renderer* parameter to ImGui_ImplSDL2_InitForSDLRenderer(), so we can use
//  SDL_GetRendererOutputSize() instead of SDL_GL_GetDrawableSize() when bound to a SDL_Renderer.
...
static void ImGui_ImplSDL2_UpdateKeyModifiers(SDL_Keymod sdl_key_mods)
{
    ...
    io.AddKeyEvent(ImGuiKey_ModCtrl, (sdl_key_mods & SDL_KMOD_CTRL) != 0);
    io.AddKeyEvent(ImGuiKey_ModShift, (sdl_key_mods & SDL_KMOD_SHIFT) != 0);
    io.AddKeyEvent(ImGuiKey_ModAlt, (sdl_key_mods & SDL_KMOD_ALT) != 0);
    io.AddKeyEvent(ImGuiKey_ModSuper, (sdl_key_mods & SDL_KMOD_GUI) != 0);
}
...
static bool ImGui_ImplSDL2_Init(SDL_Window* window, SDL_Renderer* renderer)
{
    ...
    (void)window;
        ...
        // Set SDL hint to receive mouse click events on window focus, otherwise SDL doesn't emit
        // the event. Without this, when clicking to gain focus, our widgets wouldn't activate even
        // though they showed as hovered. (This is unfortunately a global SDL setting, so enabling
        // it might have a side-effect on your application. It is unlikely to make a difference, but
        // if your app absolutely needs to ignore the initial on-focus click: you can ignore
...
#define MAP_BUTTON(KEY_NO, BUTTON_NO)                                                              \
    {                                                                                              \
        io.AddKeyEvent(KEY_NO, SDL_GetGamepadButton(game_controller, BUTTON_NO) != 0);             \
    }
